...
const Button:React.FC<ButtonProps> = ({label,onClick,disabled,outline,small,icon:Icon})=>{
    return (
        <button
            ...
            transition w-full ${outline ? 'bg-white':'bg-emerald-400'} 
            ${outline ? 'border-black':'border-emerald-400'} ${outline ? 'text-black':'text-white'}
            ${small ? 'py-1':'py-3'} ${small ? 'text-sm':'text-lg'} ${small ? 'font-light':'font-semibold'}
